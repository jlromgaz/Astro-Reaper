...
## Basic drone - slowly chases player.
...
func apply_difficulty_scale(p_scale: float) -> void:
	max_hp = HP * p_scale
	current_hp = max_hp
	DebugLog.log_info("ENEMY", "Drone scaled to %.1f HP" % max_hp)
...
func _setup_damage_area() -> void:
	...
	circle.radius = 14.0 # Slightly larger than physical body
	shape.shape = circle
	area.add_child(shape)
	add_child(area)
	area.collision_layer = 0 # No layer
	area.collision_mask = 1 # Player layer
	area.body_entered.connect(_on_body_entered)
	area.body_exited.connect(_on_body_exited)
	
	# Don't block the player physically
	collision_mask &= ~1 
...
func _on_body_entered(body: Node) -> void:
	if body.is_in_group("player"):
		...
		_player = body
		# Hit immediately if timer is at 0
		if _damage_timer <= 0:
...
func _die() -> void:
	...
	DebugLog.log_info("COMBAT", "Enemy killed at %s" % global_position)
